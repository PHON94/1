--=============================================================================
--    BANANA HUB V16 - PHIÊN BẢN ĐỒ SÁT TRIAL SIÊU TỐC & EXPAND HITBOX (FIXED)
--=============================================================================
        ...
        if _G.AutoKillPlayers then
            ...
            if target and char and hum and root and hum.Health > 0 then
                ...
                if tRoot and tHum and tHum.Health > 0 then
                    ...
                    -- FIX: Tìm remote event đúng
                    local remoteFound = false
                    local remote = nil
                    
                    -- Cách 1: Tìm trong ReplicatedStorage
                    pcall(function()
                        remote = game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Net"):FindFirstChild("RE/RegisterAttack")
                        if remote then remoteFound = true end
                    end)
                    
                    -- Cách 2: Tìm trong workspace nếu không có
                    if not remoteFound then
                        pcall(function()
                            for _, obj in pairs(game:GetService("ReplicatedStorage"):GetDescendants()) do
                                if obj.Name == "RegisterAttack" or obj.Name:find("Attack") then
                                    remote = obj
                                    remoteFound = true
                                    break
                                end
                    ...
                    -- Nếu tìm được remote
                    if remoteFound and remote then
                        ...
                        if _G.WeaveFastAttack then
                            ...
                            if fruit and melee then
                                ...
                                -- TỐC ĐỘ CAO: Spam remote 6 lần mỗi khung hình (Mạnh gấp 1.5 lần)
                                for i = 1, 6 do 
                                    pcall(function() remote:FireServer(0.4000000059604645) end)
                                end
